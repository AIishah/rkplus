    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'خیلی وقته داخل اکانت نبودی ، اگه نمونی داخل اکانت ، خودکار حذف میشه')),
        );
      }
    }
  }
  ...
  Future updateEmail({
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ScaffoldMessenger.of(context).hideCurrentSnackBar();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('مجددا وارد شوید تا ایمیل شما تغییر کند')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
                'برای ورود به اپلیکیشن نیازه که vpn یا تحریم شکن تون رو روشن کنید. اگر این کار رو کردین و باز به مشکل برخوردین به 09920833310 پیام ارسال نمائید.')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('به ایمیل شما، رمز جدید ارسال گردید')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : RanginKamanFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
